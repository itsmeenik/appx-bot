    ...
    if "start_index" not in state:
        try:
            ...
            if start_idx < 1 or start_idx > len(state["links"]):
                await message.reply_text(f"❌ Sahi number dalein (1 se {len(state['links'])}):")
                return
        ...
        if quality not in ["480p", "720p"]:
            await message.reply_text("❌ '480p' ya '720p' select karein:")
            return
...
async def process_uploads(client, message, links, start_num, total_links, quality, status_msg):
    ...
    for item in links:
        ...
        try:
            await status_msg.edit_text(f"⚙️ Live Process ({current_index}/{total_links})\n📁 File: {title}\n🎬 Quality: {quality}")
